/** Plotter.java file retrieved from: http://joshh.ug/plotter.html
  * @author Josh Hug
  * Some edits made to satisfy style checker.
  */
...
public class Plotter {
    ...
    public static void plotCategoryWeights(NGramMap ngm, WordNet wn, String[] categoryLabels,
        ...
        ChartBuilder preChart = new ChartBuilder().width(WIDTH).height(HEIGHT);
        Chart chart = preChart.xAxisTitle("years").yAxisTitle("data").build();
        ...
        for (String categoryLabel : categoryLabels) {
            ...
            if (bundle.isEmpty()) {
                System.out.println(categoryLabel + " was skipped because it had no entries.");
                continue;
            }
            if (duplicates.contains(categoryLabel)) {
                System.out.println(categoryLabel + " was found more than once.");
                continue;
            }
            count += 1;
            duplicates.add(categoryLabel);
    ...
    public static void plotAllWords(NGramMap ngm, String[] words, int startYear, int endYear) {
        ...
        for (String word : words) {
            ...
            if (bundle.isEmpty()) {
                System.out.println(word + " was skipped because it had no entries.");
                continue;
            }
            if (duplicates.contains(word)) {
                System.out.println(word + " was found more than once.");
                continue;
            }
            count += 1;
            duplicates.add(word);
    ...
    public static void plotZipfsLaw(NGramMap ngm, int year) {
        ...
        ChartBuilder preChart = new ChartBuilder().width(WIDTH).height(HEIGHT);
        Chart chart = preChart.xAxisTitle("rank").yAxisTitle("count").build();
